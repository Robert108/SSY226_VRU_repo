function [X, P, v_k, S] = kalmanFilter(Y, x_0, P_0, A, Q, H, R)
...
%First run the prediction and update with x_0 and P_0
[X(:,1), P(:,:,1)] = linearPrediction(x_0, P_0, A, Q);
[X(:,1), P(:,:,1),~,~] = linearUpdate(X(:,1), P(:,:,1), Y(:,1), H, R);

% Then loop and do the same calculations with previous state and covariance
for i=2:N
    [X(:,i), P(:,:,i)] = linearPrediction(X(:,i-1), P(:,:,i-1), A, Q);
    %[X(:,i), P(:,:,i), v_k(i), S(i)] = linearUpdate(X(:,i), P(:,:,i), Y(:,i), H, R);
